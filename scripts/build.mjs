...
function buildContext(posts, images) {
  ...
  // 图片名 → /assets/img/...（带扩展名和不带都注册）
  const imagesByName = new Map();
  for (const [rel, url] of images) {
    const full = basename(rel);
    const stem = basename(rel, extname(rel));
    if (!imagesByName.has(full)) imagesByName.set(full, url);
    if (!imagesByName.has(stem)) imagesByName.set(stem, url);
  }
  return { posts, postsByName, imagesByName };
}
...
// 行内语法：![[嵌入]] / [[双链]] / #标签
// 跳过代码块（``` 分段）和行内代码（` 分段）
function transformInline(md, ctx) {
...
function transformCallouts(md, ctx) {
...
function renderPost(post, ctx) {
  let md = transformInline(post.raw, ctx);
  md = transformCallouts(md, ctx);
...
function main() {
  ...
  // 收集图片附件：content/assets/** → /assets/img/**
  const imageFiles = walk(join(CONTENT, 'assets'));
  const images = imageFiles.map(f => [relative(join(CONTENT, 'assets'), f).replace(/\\/g, '/'),
    `/assets/img/${basename(f)}`]);
  ...
  // 文章图片附件
  if (existsSync(join(CONTENT, 'assets'))) {
    cpSync(join(CONTENT, 'assets'), join(DIST, 'assets/img'), { recursive: true });
  }
