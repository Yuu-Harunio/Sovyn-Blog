...
```bash
cd 你的项目路径        # 例如 cd d:\Code\AAstudy\Sovyn
git init
git add .
...
```json
{
  "title": "Sovyn",                     ← 站点名（标签页标题 / 导航 logo 文字）
  "description": "个人知识库博客",       ← 标签页副标题
  "url": "https://你的域名.com",        ← 改成你的真实域名（注意有 https:// ，结尾不带 /）
  "hero": {
    "heading": "首页大标题（支持 <br> 换行和 <span> 高亮）",
    "intro": "你的个人介绍",
    "tags": ["你的", "标签"],
    "image": "/assets/img/hero.png",    ← 首页右侧插图（不想显示就删掉这行）
    "alt": "插图描述文字"
  },
  "footer": {
    "left": "© 2026 你的名字 · 一句话",  ← 页脚左下角
    "links": [
      { "label": "GitHub", "url": "https://github.com/你/仓库" }  ← 页脚右下角链接
    ]
  }
}
```

> 换图标 / 首页插图：直接替换 `assets/img/icon.png`（favicon + 导航 logo 共用）和 `assets/img/hero.png`，不用改代码。
...
| 字段 | 必填 | 作用 |
| :--- | :--- | :--- |
| `title` | 推荐 | 显示的标题（不写则用文件名）。注意：文章页的大标题由它渲染，正文开头不要再写同名 H1，否则会显示两个标题 |
| `date` | 推荐 | 排序依据，格式 `2026-08-30` |
| `tags` | 可选 | 标签列表，会自动生成标签聚合页。**建议三个**：文章自身的核心知识 + 一个大类别（编程 / 工具 / 硬件 / 笔记方法）+ 一个联动标签（Agent / WSL / Linux 等），保证每个标签点进去都有至少 2 篇 |
| `summary` | 可选 | 首页列表和引用卡片的摘要。单行为宜，写纯文本（不要用 `[[双链]]` 语法，列表页不解析它） |
...
把 `publish: true` 改成 `publish: false`（或删掉 `content/posts/` 里对应的 .md 文件），push 即可。

> 放心：文章下线时，**它引用的图片附件也会一起从公开站消失**（构建器只拷贝已发布文章用到的图片），正文里指向它的 `[[双链]]` 会自动降级成灰色文字，不会出现死链。但注意：如果这些图片曾被 push 上过线，它们仍留在 Git 历史记录里——想彻底清除需要改写 Git 历史。

### 发布前的小提醒（隐私检查）

文章 push 上线前扫一眼正文和截图：真实 WiFi 名称 / 路由器 MAC（BSSID）、设备 MAC、内网 IP（192.168.x.x）、电脑账户名、激活序列号截图等，能换占位符就换。细节见 MAP.md「当前站点状态速览」。
...
① 图片没拷进项目（`.assets` 文件夹要和 .md 一起拷）；② `![[图片名]]` 里的名字和文件名不一致；③ 标准写法 `![](路径)` 的路径不对；④ **所在文章没写 `publish: true`**——只有已发布文章引用的图片才会被拷进站点。检查 `dist/assets/img/` 里有没有这张图。
...
**当前状态（2026-08-30）**：博客已上线（`https://sovyn.eu.cc`，仓库 `https://github.com/Yuu-Harunio/Sovyn-Blog`），当前 18 篇文章发布中。日常你只需要第五节的流程：写笔记 → 拷进 `content/posts/` → 加 frontmatter → push。技术细节和排障定位见 MAP.md。
